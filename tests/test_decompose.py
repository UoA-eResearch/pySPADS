...
from pySPADS.processing.data import (
    _interpolate,
    load_data_from_csvs,
    imf_filename,
    parse_filename,
...
def test_load_separate_files():
    ...
    assert len(data) == len(
        expected_series
    ), f"Expected {len(expected_series)} series, got {len(data)}"
...
def test_decompose():
    ...
    imf_df = steps.decompose(
        data, noise=0.1, num_trials=100, progress=False, parallel=False
    )
    ...
    for i, col in enumerate(imf_df.columns):
        assert isinstance(
            col, int
        ), f"Expected column to be integer, got {col} of type {type(col)}"
...
def test_imf_filename():
    ...
    assert (
        filename == output_dir / "shore_imf_0.123456789.csv"
    ), "Incorrect formatted filename"
...
def test_parse_filaname():
    ...
    for label, noise in itertools.product(
        ["shore", "test", "a_", "_b", "_c__"], [0, 0.1, 0.123456789]
    ):
